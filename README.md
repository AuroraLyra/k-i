...
- 世界书分为线上全局、线下全局、局部世界书；每本世界书支持多个可单独开关的条目、绿灯关键词触发、蓝灯常驻注入和黄灯优先注入，局部世界书可绑定到角色。系统固定保留不可删除、不可停用的“禁忌之书”：非空启用条目会作为最高优先级上下文置于全站文本生成与图片生成请求最前方，内容全空时不注入；备份恢复或清空世界书数据后仍会保留空书。
