import { createBackupTemporaryFileWriter, type BackupTemporaryFile } from '@/utils/backupTemporaryFile';
...
export async function encryptLinkBackupFile(source: Blob, recoveryKey: string, exportedAt = Date.now(), onProgress?: BackupCryptoProgress): Promise<BackupTemporaryFile> {
  validateRecoveryKey(recoveryKey);
  const salt = crypto.getRandomValues(new Uint8Array(16));
  const key = await deriveEncryptionKey(recoveryKey.trim(), salt, encryptedBackupIterations);
  const payloadWriter = await createBackupTemporaryFileWriter('link-backup-encrypted-payload');
  const chunks: EncryptedLinkBackupEnvelope['chunks'] = [];
  const totalChunks = Math.max(1, Math.ceil(source.size / encryptedBackupChunkBytes));
  let payload: BackupTemporaryFile | null = null;
  ...
  try {
    for (let index = 0; index < totalChunks; index += 1) {
      ...
      await payloadWriter.write(new Blob([encrypted], { type: 'application/octet-stream' }));
      chunks.push({ iv: bytesToBase64(iv), plaintextBytes: plaintext.byteLength, encryptedBytes: encrypted.byteLength });
      await onProgress?.(Math.round(((index + 1) / totalChunks) * 80));
    }
    payload = await payloadWriter.close();
    const envelope: EncryptedLinkBackupEnvelope = {
      app: 'LINK',
      encryptedBackupVersion: 2,
      exportedAt,
    ...
    const header = new TextEncoder().encode(JSON.stringify(envelope));
    const outputWriter = await createBackupTemporaryFileWriter('link-backup-encrypted');
    try {
      await outputWriter.write(new Blob([
        toArrayBuffer(encryptedBackupMagic),
        encodeHeaderLength(header.byteLength),
        toArrayBuffer(header)
      ], { type: 'application/vnd.babylink.encrypted-backup' }));
      for (let offset = 0; offset < payload.file.size; offset += encryptedBackupChunkBytes) {
        const end = Math.min(payload.file.size, offset + encryptedBackupChunkBytes);
        await outputWriter.write(payload.file.slice(offset, end));
        await onProgress?.(80 + Math.round(end / Math.max(1, payload.file.size) * 20));
      }
      return await outputWriter.close();
    } catch (error) {
      await outputWriter.abort();
      throw error;
    }
  } catch (error) {
    await payloadWriter.abort().catch(() => undefined);
    throw error;
  } finally {
...
export async function decryptLinkBackupFile(blob: Blob, recoveryKey: string, onProgress?: BackupCryptoProgress): Promise<BackupTemporaryFile> {
  validateRecoveryKey(recoveryKey);
  const prefixLength = encryptedBackupMagic.byteLength + 4;
  if (blob.size < prefixLength) throw new Error('云端文件不是有效的 BabyLink 加密备份。');
  const prefix = new Uint8Array(await blob.slice(0, prefixLength).arrayBuffer());
  ...
  if (!Number.isInteger(envelope.kdf.iterations) || envelope.kdf.iterations < 100_000 || envelope.kdf.iterations > 2_000_000) {
    throw new Error('云端备份的密钥参数无效。');
  }

  const outputWriter = await createBackupTemporaryFileWriter('link-backup-decrypted');
  try {
    const key = await deriveEncryptionKey(recoveryKey.trim(), base64ToBytes(envelope.kdf.salt), envelope.kdf.iterations);
    let offset = prefixLength + headerLength;
    let plaintextBytes = 0;
    for (let index = 0; index < envelope.chunks.length; index += 1) {
      const chunk = envelope.chunks[index];
      ...
      if (decrypted.byteLength !== chunk.plaintextBytes) throw new Error('云端备份分块大小校验失败。');
      await outputWriter.write(new Blob([decrypted], { type: 'application/octet-stream' }));
      plaintextBytes += decrypted.byteLength;
      offset += chunk.encryptedBytes;
      await onProgress?.(Math.round(((index + 1) / envelope.chunks.length) * 100));
    }
